...
quantites, noms, periodes). Fonctions pures, reutilisees par les FieldType
(voir field_types.py et tools/*.py) pour interpreter une reponse de
clarification. Portee depuis assistantv2/parsing.py ; `coerce_params` n'existe
plus ici, remplacee par FieldType.coerce (voir registry.py) : la conversion
JSON-safe -> type riche est desormais la responsabilite du type de champ, pas
d'une fonction generique keyee par nom de champ.
"""
import re
import unicodedata
...
def parse_period(text: str) -> tuple[date, date, str] | None:
    ...
    Semaine calee sur lundi. Dates calendaires locales : le deploiement
    actuel est a Dakar (UTC+0, sans heure d'ete), equivalent a UTC.
    """
    norm = normalize(text)
    today = date.today()
