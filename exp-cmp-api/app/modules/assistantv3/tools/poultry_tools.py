"""Business poulets (poulailler) : declare son module (alias) et ses outils.
N'a besoin d'aucun type de champ propre : quantite/montant/compte/categorie
sont deja fournis par field_types.py."""
import uuid
from decimal import Decimal

from app.modules.assistantv3 import resolvers
from app.modules.assistantv3.registry import BusinessModule, ToolSpec, register, register_business
...
def _register() -> None:
    register_business(BusinessModule(
        code=BUSINESS_CODE,
        aliases={"poulet", "poulailler", "poulaillers", "poules", "poule"},
        context_hint="Gestion d'un poulailler : achats et ventes de poulets par lots, suivi de stock.",
    ))
    ...
    register(ToolSpec(
        ...
        field_type_overrides={"category": "category_debit"},
        is_critical=True,
        is_read_only=False,
    ))
    register(ToolSpec(
        ...
        field_type_overrides={"category": "category_credit"},
        is_critical=True,
        is_read_only=False,
    ))
    register(ToolSpec(
